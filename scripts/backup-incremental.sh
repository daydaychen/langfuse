...
# 检查参数或读取同步指针
if [ -z "$1" ]; then
    if [ -f "${SOURCE_DIR}/.sync_pointer" ]; then
        LAST_BACKUP_HASH=$(cat "${SOURCE_DIR}/.sync_pointer")
        echo -e "${YELLOW}从.sync_pointer读取上次备份: $LAST_BACKUP_HASH${NC}"
    else
        echo -e "${RED}错误: 请指定上次备份的commit哈希${NC}"
        echo "使用方法: ./scripts/backup-incremental.sh <上次备份的commit哈希>"
        echo "或者确保.sync_pointer文件存在"
        exit 1
    fi
else
    LAST_BACKUP_HASH="$1"
fi
PATCH_NAME="langfuse-incremental-${TIMESTAMP}.patch"
...
echo "3. 下次增量备份时，脚本会自动读取.sync_pointer文件"